...
# 新增的欧式距离计算函数
def compute_euclidean_distance(query_feat, gall_feat):
    """
    计算查询特征和图库特征之间的欧式距离矩阵
    
    参数:
        query_feat: (nquery, feat_dim) 查询特征矩阵
        gall_feat: (ngall, feat_dim) 图库特征矩阵
    
    返回:
        distmat: (nquery, ngall) 欧式距离矩阵，值越小表示越相似
    """
    # 方法1: 使用广播和矩阵运算（高效但内存消耗大）
    # distmat = np.sqrt(np.sum((query_feat[:, np.newaxis, :] - gall_feat[np.newaxis, :, :]) ** 2, axis=2))
